...
export const store = new Vuex.Store({
  ...
  mutations: {
    ...
    setInterceptionsData (state, payload){
      state.interceptionsData = payload
    },
...
//     ===================
    ...
    fetchTotalInterceptions({commit}){
      ...
      // Getting Interceptions Data
      // Data Interceptions
    fetchInterceptionsData({commit}){
      // Fetching List of Dates
      firebase.firestore().collection('storedata').onSnapshot((report) => {
        // Creating a temporary Variable
        let totalInterceptions = 0;
        ...
        console.log( 'Total Number of Interception' ,totalInterceptions)
        console.log( 'Interception Details' ,interceptionData)
        commit('setInterceptionsData', totalInterceptions);
      })
    },
    // ==============
    // Fetching Data
    ...
    storeListUPD({commit}){
      commit('SET_MAIN_LOADING', true);
      let cities = {};
      firebase.firestore().collection('stores').onSnapshot((storelist) => {
          const stores = [];
          let entries = {};
          let totalStores = 0;
          // Grabing Cities
    ...
    baListUPD({commit}){
      commit('SET_MAIN_LOADING', true);
      firebase.firestore().collection('app-users').onSnapshot((balist) => {
          let ba = [];
          // const obj = balist.data();
          let totalBA = 0;
          // Creating References
          balist.forEach((baData) => {
              ba.push(baData.data());
              totalBA = totalBA + 1;
          });
          commit('SET_MAIN_LOADING', false);
  ...
  // },
    ...
    fetchAllStoreReports({commit, getters}){
      ...
      firebase.firestore().collection('storedata').onSnapshot((report) => {
        ...
        for (let key in questionParameters){
            questionsList[questionParameters[key].id] = questionParameters[key].answers;
        ...
        report.forEach((recentReports) => {
          ...
          // Creating variables in Answers
          for (let key in questionParameters) {
              answersReading[questionParameters[key].id] = [];
              answersReading[questionParameters[key].id][questionParameters[key].answers] = 0
          ...
          questionName.forEach((name) => {
              ...
              for (let qkey in questionsList) {
                  ...
                  // console.log('questionsList from iterator =>', questionsList );
              }
          ...
          parametersName.forEach((parameter) => {
            // this loop is returning names
              optionals.forEach((option) => {
                  ...
                  // else if (option[parameter] == null || option[parameter] == '' ){
                  //     parametersList[parameter].no++;
                  // }
              })
          });
        ...
        // Making Names Variables
          questionName.forEach((name) => {
              ...
              // console.log('Question name =>', questionName + 'Question' + question);
              // questions.forEach((answer) => {
              //     console.log('Question Data =>', questions + 'Answers' + answer)
    ...
    storeReg({commit}, payload){
      ...
      firebase.firestore().collection('stores').add(payload).then(() => {
        // End Loading
        commit('SET_MAIN_LOADING', false)
        // Sending Success Message
    ...
    // new Brand Ambassador Reg
    baReg({commit}, payload){
      // Start Loading
      commit('SET_MAIN_LOADING', true);
      firebase.firestore().collection('app-users').add(payload).then(() => {
          // End Loading
          commit('SET_MAIN_LOADING', false)
          // Sending Success Message
